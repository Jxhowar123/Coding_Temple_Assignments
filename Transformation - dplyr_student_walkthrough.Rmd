...
filter(nycflights13::flights, month == 3, day ==15)
...
```{r}
view(filter(nycflights13::flights, carrier == "AA" | carrier == "UA" |carrier == "DL"))
...
```{r}
view(arrange(flight_data,carrier, desc(sched_dep_time)))
```

## Selecting Columns

```{r}
 select(flight_data,year,month,day)
```
...
-   starts_with()
    ...
    ```{r}
    view(select(flight_data, ends_with(("ar"))))
    ```
...
-   everything() will include all the rest of the columns. So you can put it at the end of a select statement as a parameter, and you'll be able to move selected columns to the left side, and then include the rest of the data.
    ...
    ```{r}
    select(flight_data,time_hour, air_time,everything())
    ```
...
```{r}
flight_info <- select(flight_data,
                      year: day,
                      ends_with("delay"),
                      distance,
                      air_time
...
view(mutate(flight_info,
            goin = arr_delay - dep_delay,
...
```{r}
summarize(flight_data, delay = mean(dep_delay, na.rm = TRUE))
```
...
```{r}
by_day <- group_by(flight_data, year, month, day)
...
by_destination <- group_by(flight_data, dest)
